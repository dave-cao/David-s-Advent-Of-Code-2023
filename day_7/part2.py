...
def get_card_count(cards):
    ...
    print(card_count)
    # get max count of highest card
    max = 0
    max_card = "J"
    for card, count in card_count.items():
        if count > max and card != "J":
            max = count
            max_card = card
        elif count == max and CARDS.index(card) > CARDS.index(max_card):
            max = count
            max_card = card

    print(card_count, max_card)

    if card_count.get("J") is not None:
        if max_card != "J":
            card_count[max_card] = card_count.get(
                max_card) + card_count.get("J")
            del card_count["J"]
